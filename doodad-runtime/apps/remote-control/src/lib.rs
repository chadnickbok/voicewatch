...
use doodad_sdk::{decode_ui_event, mount_appspec, request_remote};

const HOME: &[u8] = include_bytes!("../appspec.cbor");
const TARGETS: &[u8] = include_bytes!("../screens/targets.cbor");
const PENDING: &[u8] = include_bytes!("../screens/pending.cbor");
const DONE: &[u8] = include_bytes!("../screens/done.cbor");
const OFFLINE: &[u8] = include_bytes!("../screens/offline.cbor");

fn target_for(action_id: &str) -> Option<&'static [u8]> {
    match action_id {
        "remote-control.primary" => Some(TARGETS),
        "remote.camera" => Some(PENDING),
        "remote.disconnect" => Some(OFFLINE),
        "remote.ack" => Some(DONE),
        "remote.targets" => Some(TARGETS),
        "remote.reconnect" => Some(TARGETS),
        "remote.home" => Some(HOME),
        _ => None,
    }
}
...
pub unsafe extern "C" fn handle_event(
    ...
    if request_remote(event.action_id, &[]).is_err() {
        return 0;
    }
