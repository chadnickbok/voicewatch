...
use doodad_sdk::{decode_ui_event, mount_appspec, request_sensor};
...
const EXPORT: &[u8] = include_bytes!("../screens/export.cbor");
const EXPORTED: &[u8] = include_bytes!("../screens/exported.cbor");

fn target_for(action_id: &str) -> Option<&'static [u8]> {
    match action_id {
        "sensor-recorder.primary" => Some(RECORDING),
        "sensor.pause" => Some(PAUSED),
        "sensor.finish" => Some(EXPORT),
        "sensor.resume" => Some(RECORDING),
        "sensor.export" => Some(EXPORT),
        "sensor.export.csv" => Some(EXPORTED),
        "sensor.again" => Some(RECORDING),
        "sensor.session" => Some(EXPORT),
        "sensor.home" => Some(HOME),
        _ => None,
    }
}
...
pub unsafe extern "C" fn handle_event(
    ...
    if request_sensor(event.action_id, &[]).is_err() {
        return 0;
    }
