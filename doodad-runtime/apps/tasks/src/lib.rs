...
const TODAY_THREE: &[u8] = include_bytes!("../screens/list-three.cbor");
...
impl Runtime {
    ...
    fn render(&mut self) -> u64 {
        ...
        if self.commands.begin(command_count).is_err()
            ...
            || self
                .commands
                .set_checked("tasks.milk", self.milk_done)
                .is_err()
            || self
                .commands
                .set_checked("tasks.coffee", self.coffee_done)
                .is_err()
        {
            return 0;
        }
        if self.bananas
            && self
                .commands
                .set_checked("tasks.bananas", self.bananas_done)
                .is_err()
        {
            return 0;
        }
...
pub unsafe extern "C" fn handle_event(
    ...
    match event.action_id {
        "tasks.add.bananas" => {
            ...
            let _ = mount_appspec(TODAY_THREE);
            runtime.render()
        }
        "tasks.toggle.milk" => {
