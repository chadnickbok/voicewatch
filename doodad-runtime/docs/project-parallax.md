...
- [x] Those stages represent 75 distinct screen IDs and 93 distinct semantic
  states.
...
- [x] The generated inventory covers 83 documents, 105 checkpoints, 114
...
- [ ] LVGL still has root flex-axis, touch-target, typography, max-lines,
  ...
  path.
- [ ] Only 20 of 105 checkpoints have dual-renderer captures.
- [ ] The fast Compose lane still uses Robolectric SDK 33; installed API 37
  AVDs have not yet produced reviewed runtime baselines.
...
| `button` | `Button`, `TextButton`, or `CompactButton` by role | 80 documents | [x] |
| `card` | `Card`/`TitleCard` by content role | 71 documents | [x] |
...
- [x] Counted the current coverage: 83 AppSpec documents, 105 flow stages,
  75 screen IDs, and 93 semantic states.
...
- [x] Recorded 20 decisive traces containing 105 checkpoints, 114 accepted
