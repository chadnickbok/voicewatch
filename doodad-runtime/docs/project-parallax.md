...
| Field | Value |
|---|---|
| Document role | Dedicated dual-renderer conformance tracker; not the overall roadmap |
| Status | Active maintenance tracker; trace/replay foundation complete, review and hardware closure partial |
...
Overall product sequencing lives in the [Doodad roadmap](roadmap.md).
...
| P4 | AppSpec-to-Compose reference renderer | Static renderer complete and exercised by reviewed app flows |
| P5 | Twenty state-aligned initial app comparisons | Nineteen apps reviewed and approved; Snake pending |
...
| P9 | Reviewed API 37 runtime authority | Twenty apps captured; nineteen reviewed, with Snake pending |
...
- [x] 101 of 105 checkpoints have reviewed dual-renderer captures and recorded
  app-level dispositions.
- [ ] Snake's four checkpoints still need reviewed dual-renderer and API 37
  evidence.
- [ ] Cross-suite variants, motion evidence, the live parallel-rendering
  simulator, CI approval policy, and linked hardware evidence remain open.
...
**Status:** Trace corpus and LVGL replay complete; Compose coverage, review,
variants, and motion pending · **Engineering size:** Large

### Existing decisive flows

- [x] Record all twenty flows as SceneTrace v1 under `reference/traces/`.
- [x] Render all 105 stages in LVGL replay with matching live/replay
  attestations.
- [ ] Render every visually distinct accepted stage in Compose.
- [x] Deduplicate identical scene hashes without losing timeline causality; the
  inventory records 96 unique snapshots across 105 checkpoints.
- [ ] Review all 96 currently distinct semantic states.
